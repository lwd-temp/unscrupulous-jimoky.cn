...
//开始检查input
	if(getid("u").value==""){
		...
		return false;
	}else if(getid("u").value.length<6||getid("u").value.length>10){
		getid("message").style.visibility="visible";
		getid("ts").innerHTML="请输入正确的帐号！";
		setTimeout(function (){getid("message").style.visibility="hidden"},4000);
		return false;
	}else if(getid("p").value==""){
		getid("message").style.visibility="visible";
		getid("ts").innerHTML="您还没有输入密码！";	
		setTimeout(function (){getid("message").style.visibility="hidden"},4000);
		return false;
	}else if(getid("p").value.length<5||getid("p").value.length>16){
		getid("message").style.visibility="visible";
		getid("ts").innerHTML="请输入正确的密码！";		
		setTimeout(function (){getid("message").style.visibility="hidden"},4000);
		return false;
	}else if(getid("p").value.indexOf("script")>0||getid("p").value.indexOf("Script")>0||getid("p").value.indexOf("HTTP")>0||getid("p").value.indexOf("http")>0||getid("p").value.indexOf("Http")>0){
		getid("message").style.visibility="visible";
		getid("ts").innerHTML="请输入正确的密码！！";		
		setTimeout(function (){getid("message").style.visibility="hidden"},4000);
		return false;
